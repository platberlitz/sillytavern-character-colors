(async function() {
    ...
    function scanForColors(element) {
        ...
        const blocklist = new Set(['she','he','they','it','i','you','we','her','him','them','his','hers','its','their','theirs','one','someone','anyone','everyone','nobody','somebody','anybody','everybody','the','a','an','this','that','these','those','what','who','where','when','why','how','something','nothing','everything','anything','dark','light','secret','puzzle','toward','towards','about','over','under','through','between','around','behind','before','after','during','and','but','or','nor','for','yet','so','if','then','than','because','although','though','while','unless','until','since','once','where','wherever','whereas','whether','which','whatever','whichever','whoever','however','moreover','furthermore','therefore','thus','hence','accordingly','consequently','meanwhile','nonetheless','nevertheless','otherwise','instead','regardless','despite','beyond','within','without','upon','onto','into','from','with','your','my','our','their','some','any','no','every','each','all','both','few','many','most','other','another','such','only','own','same','just','even','still','also','very','too','quite','rather','really','almost','already','always','never','often','sometimes','usually','here','there','now','then','today','tomorrow','yesterday','soon','later','again','back','away','down','up','out','off','well','much','more','less','first','last','next','new','old','good','bad','great','little','big','small','long','short','high','low','young','right','left','hand','hands','eyes','eye','face','head','voice','door','room','way','time','day','night','morning','evening','world','life','man','woman','people','thing','things','place','year','years','work','part','case','point','fact','home','water','money','story','being','having','going','looking','making','coming','taking','getting','using','finding','giving','telling','asking','trying','leaving','calling','feeling','thinking','knowing','seeing','wanting','meaning','seeming','needing','turning','beginning','keeping','holding','bringing','writing','standing','sitting','running','walking','talking','living','working','playing','moving','waiting','watching','reading','opening','closing','following','stopping','starting','happening','including','continuing','considering','appearing','remaining','becoming','growing','falling','rising','lying','dying','setting','cutting','putting','letting','meeting','paying','saying','hearing','speaking','showing','leading','learning','changing','building','sending','receiving','understanding','developing','creating','providing','adding','producing','reaching','spending','losing','winning','breaking','driving','buying','wearing','choosing','drawing','pushing','pulling','carrying','picking','throwing','hitting','killing','fighting','eating','drinking','sleeping','waking','loving','hating','hoping','wishing','fearing','believing','remembering','forgetting','imagining','wondering','realizing','noticing','recognizing','supposing','assuming','expecting','planning','deciding','agreeing','refusing','offering','promising','threatening','warning','advising','suggesting','recommending','ordering','demanding','requesting','begging','insisting','admitting','denying','claiming','arguing','explaining','describing','mentioning','reporting','announcing','declaring','stating','expressing','indicating','revealing','proving','confirming','supporting','accepting','rejecting','approving','disapproving','criticizing','praising','blaming','thanking','apologizing','forgiving','excusing','congratulating','welcoming','greeting','introducing','presenting','representing','replacing','exchanging','comparing','contrasting','combining','separating','connecting','disconnecting','attaching','detaching','joining','dividing','multiplying','subtracting','calculating','measuring','counting','weighing','testing','checking','examining','inspecting','investigating','researching','studying','analyzing','evaluating','assessing','judging','rating','ranking','sorting','organizing','arranging','preparing','planning','designing','constructing','manufacturing','producing','processing','handling','managing','controlling','operating','maintaining','repairing','fixing','adjusting','modifying','improving','upgrading','updating','installing','removing','deleting','copying','pasting','saving','loading','downloading','uploading','printing','scanning','recording','playing','pausing','stopping','rewinding','forwarding','skipping','repeating','shuffling','muting','unmuting','increasing','decreasing','maximizing','minimizing','expanding','collapsing','zooming','scrolling','clicking','tapping','swiping','dragging','dropping','selecting','highlighting','underlining','bolding','italicizing','formatting','editing','revising','correcting','proofreading','reviewing','approving','publishing','distributing','sharing','posting','commenting','liking','disliking','subscribing','unsubscribing','following','unfollowing','blocking','unblocking','reporting','flagging','hiding','showing','displaying','presenting','demonstrating','illustrating','explaining','clarifying','simplifying','complicating','confusing','misleading','deceiving','lying','cheating','stealing','robbing','murdering','assaulting','attacking','defending','protecting','guarding','securing','locking','unlocking','encrypting','decrypting','encoding','decoding','compressing','decompressing','archiving','extracting','backing','restoring']);
        
        const html = mesText.innerHTML;
        const fontRegex = /<font\s+color=["']?#([a-fA-F0-9]{6})["']?[^>]*>([\s\S]*?)<\/font>/gi;
        let match, foundNew = false;
        
        // Speech verbs for high-confidence detection
        const speechVerbs = /\b(said|says|replied|replies|asked|asks|whispered|whispers|yelled|yells|shouted|shouts|exclaimed|exclaims|murmured|murmurs|muttered|mutters|answered|answers|called|calls|cried|cries|chirped|chirps|purred|purrs|announced|announces|spoke|speaks|stated|states|remarked|remarks|commented|comments|explained|explains|declared|declares|demanded|demands|warned|warns|laughed|laughs|sighed|sighs|groaned|groans|growled|growls|hissed|hisses|snapped|snaps|screamed|screams|mumbled|mumbles|breathed|breathes|gasped|gasps|huffed|huffs|scoffed|scoffs)\b/i;
        ...
        while ((match = fontRegex.exec(html)) !== null) {
            ...
            const beforeTag = html.substring(Math.max(0, tagStart - 250), tagStart);
            const afterTag = html.substring(tagEnd, Math.min(html.length, tagEnd + 150));
            
            let speaker = null;
            
            // Method 1: Speech verb pattern (high confidence)
            const beforeVerb = beforeTag.match(/([A-Z][a-z]{2,})\s+(?:said|says|replied|replies|asked|asks|whispered|whispers|yelled|yells|shouted|shouts|exclaimed|exclaims|murmured|murmurs|muttered|mutters|answered|answers|called|calls|cried|cries|chirped|chirps|purred|purrs|announced|announces|spoke|speaks|stated|states|remarked|remarks|commented|comments|explained|explains|declared|declares|demanded|demands|warned|warns|laughed|laughs|sighed|sighs|groaned|groans|growled|growls|hissed|hisses|snapped|snaps|screamed|screams|mumbled|mumbles|breathed|breathes|gasped|gasps|huffed|huffs|scoffed|scoffs)[,.:]*\s*["'"「『«]?\s*$/i);
            if (beforeVerb) speaker = beforeVerb[1];
            
            if (!speaker) {
                const afterVerb = afterTag.match(/^["'"」』»]?[,.]?\s*([A-Z][a-z]{2,})\s+(?:said|says|replied|replies|asked|asks|whispered|whispers|yelled|yells|shouted|shouts|exclaimed|exclaims|murmured|murmurs|muttered|mutters|answered|answers|called|calls|cried|cries|chirped|chirps|purred|purrs|announced|announces|spoke|speaks|stated|states|remarked|remarks|commented|comments|explained|explains|declared|declares|demanded|demands|warned|warns|laughed|laughs|sighed|sighs|groaned|groans|growled|growls|hissed|hisses|snapped|snaps|screamed|screams|mumbled|mumbles|breathed|breathes|gasped|gasps|huffed|huffs|scoffed|scoffs)/i);
                if (afterVerb) speaker = afterVerb[1];
            }
            
            // Method 2: Find closest capitalized name before the dialogue (within same sentence/paragraph)
            if (!speaker) {
                // Look for "Name verb," or "Name's" pattern close to the quote
                const closeMatch = beforeTag.match(/([A-Z][a-z]{2,})(?:'s)?\s+[a-z]+[^.!?]*["'"「『«]\s*$/i);
                if (closeMatch && !blocklist.has(closeMatch[1].toLowerCase())) {
                    speaker = closeMatch[1];
                }
            }

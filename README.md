...
- **"Colors saved" keeps reverting to Per card**: versions before 5.9.5 rebuilt the stored settings snapshot with a storage scope in it even when the saved data had never recorded one, so a settings record written before the setting existed was stamped with Per card the first time it was read, and any partial snapshot applied over the live one reset the choice. The scope is now only read back when it was actually saved. If your setting had already been overwritten, set it once more and it will stick.

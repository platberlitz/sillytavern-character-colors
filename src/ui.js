...
// One of the user's other personas: it has a pinned color of its own. Only shown while
// the pins are live, since that is when the entry actually behaves differently.
export function isPinnedPersonaEntry(entry) {
    if (settings.persistPersonaColor !== true || !entry || isPersonaEntry(entry)) return false;
    return Object.values(getPinnedPersonaColors()).some(pinned => {
        const identity = normalizeRegistryIdentity(String(pinned?.name ?? ''));
        return !!identity && matchesPersonaIdentity(entry, identity);
    });
}
...
export function buildCharRowHtml(k, v) {
    ...
    const statusBadges = [
        ...
        isPinnedPersonaEntry(v) ? '<span class="dc-status-chip dc-status-chip-persona" title="One of your personas; its color is kept everywhere">Persona</span>' : '',
...
export function syncUIWithSettings() {
    ...
    if ($('dc-keep-card-character')) $('dc-keep-card-character').checked = settings.keepCardCharacter !== false;
            ...
            <details class="dc-section" id="dc-page-engine" data-dc-page="engine" data-dc-disclosure="engine" role="tabpanel" aria-labelledby="dc-tab-engine" tabindex="-1">
                ...
                <div class="dc-stack">
                    <div class="dc-llm-only dc-stack">
                        <div class="dc-field-row"><label class="dc-inline-label" for="dc-llm-profile">Colorize profile</label><select id="dc-llm-profile" class="text_pole" data-help="Used by the Colorize button, automatic colorizing and palette generation. While Prompt mode is set to inject or macro, the reply model colors as it writes and this profile is never called; set Prompt mode to &quot;Use the colorize profile&quot; to hand the coloring to it."><option value="">Use main chat AI</option></select></div>
                        ...
                        <div class="dc-field-row"><label class="dc-inline-label" for="dc-prompt-mode">Prompt mode</label><select id="dc-prompt-mode" class="text_pole" aria-describedby="dc-prompt-mode-note" data-help="Inject adds the coloring instruction to the reply prompt. Macro delivers it only where you put {{dialoguecolors}} yourself. &quot;Use the colorize profile&quot; sends nothing to the reply model and colors each reply afterwards, in a separate request on the Colorize profile."><option value="inject">Inject automatically</option><option value="macro">Use macro manually</option><option value="profile">Use the colorize profile</option></select></div><small id="dc-prompt-mode-note" style="display:none;"></small>
                    ...
                    <div class="dc-dom-only dc-stack" style="display:none;">
                        ...
                        <div class="dc-field-row"><label class="dc-inline-label" for="dc-attr-profile">Verify profile</label><select id="dc-attr-profile" class="text_pole" data-help="Used only by attribution verification, which checks who each colored line belongs to. It has no effect on the coloring itself."><option value="">Use main chat AI</option></select></div><div class="dc-field-row"><label class="dc-inline-label" for="dc-attr-max-tokens">Verify token limit</label><input type="number" id="dc-attr-max-tokens" min="256" max="32768" value="4096" class="text_pole"></div>
...
function bindSettingsPanelControls($) {
    ...
    $('dc-keep-card-character').onchange = e => {
        settings.keepCardCharacter = e.target.checked;
        if (e.target.checked) markCardCharactersKept();
        saveData();
        updateCharList();
    };
    $('dc-persist-persona-color').onchange = e => {
        settings.persistPersonaColor = e.target.checked;
        // Pin whatever the personas look like right now, so enabling the option keeps the
        // colors the user is already seeing instead of waiting for the next edit.
        if (e.target.checked) syncPinnedPersonaColors();
        saveData();
        updateCharList();
    };
    ...
    $('dc-llm-profile').onchange = e => { settings.llmConnectionProfile = e.target.value || null; saveData(); updateSystemPromptDisplay(); };
